...
#' For example the current and previous value for a table column/entry as well as ratios and percentage differences.
...
#' the ratio of the previous and current value and the percentage difference.
#'
#'
#' @param table The table to extract information from. This is assumed to be a dataframe and in tidy format
#' @param current_pub_year This is the current publication year
...
#'
#' tot_cases_current --> contains the current quarter values for each of the columns in the main dataframe
#'
#' tot_cases_previous --> contains the previous quarter values for each of the columns in the main dataframe
#'
#' tot_cases_change   --> contains the change between the current quarter values and the previous one
#'
#' tot_cases_ratio   --> contains the ratio of the current and previous quarter divided by the value of the previous quarter
#'
#' tot_cases_perc_change   --> contains the percentage change between the current and previous quarter values
#'
#' @examples
#'
...
calculations = function(table, current_pub_year , current_pub_quarter ){
  ...
  tot$tot_cases_current <- table  %>%
    filter(year == current_pub_year & quarter == paste0("Q", current_pub_quarter)) %>%
    as.vector() %>%
  ...
  tot$tot_cases_previous <- table %>%
    filter(year == current_pub_year-1 & quarter == paste0("Q",current_pub_quarter)) %>%
    as.vector() %>%
    select(-c("year", "quarter")) %>%
    as.numeric() %>%
    enframe() %>%
